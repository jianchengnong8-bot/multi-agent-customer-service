    ...
    async def synthesize_response(self, state: AgentState) -> AgentState:
        ...
        if not compliance_passed:
            ...
            for agent_name, result in sub_results.items():
                if not result:
                    continue
                # 子 Agent 可能返回 str 或 dict，统一提取文本
                if isinstance(result, str):
                    result_parts.append(result)
                elif isinstance(result, dict):
                    # 优先取 answer/response/summary 字段，否则序列化整个 dict
                    text = result.get("answer") or result.get("response") or result.get("summary")
                    if text:
                        result_parts.append(str(text))
                    # 跳过纯元数据 dict（如 compliance/intent_router）
