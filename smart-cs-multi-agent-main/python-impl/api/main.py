...
# 确保项目根目录在 sys.path 中，支持 python api/main.py 方式启动
...
# 结构化日志（符合 CLAUDE.md：禁止 print，使用 logging + JSON）
logging.basicConfig(
    level=logging.INFO,
    format='{"time": "%(asctime)s", "level": "%(levelname)s", "module": "%(name)s", "message": "%(message)s"}',
)
logger = logging.getLogger("smart-cs")
...
# 前端测试页面内容（模块加载时读取一次，避免每次请求读盘）
_STATIC_DIR = Path(__file__).resolve().parent.parent / "static"
_INDEX_HTML = (_STATIC_DIR / "index.html").read_text(encoding="utf-8") if (_STATIC_DIR / "index.html").exists() else "<h1>index.html not found</h1>"
...
async def chat(request: ChatRequest):
    ...
    try:
        ...
        logger.error(f"Graph execution failed: {type(e).__name__}: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"处理失败: {type(e).__name__}: {str(e)}")
